#!/usr/bin/env node
/**
 * Parse F1-Data-2022.csv (Japanese format) and convert to standard 3-file CSV format
 *
 * Usage: pnpm tsx server/scripts/parseF1Data2022.ts
 */
...
// Driver name to driver code mapping (2022 uses English names already)
const driverCodeMap: Record<string, string> = {
  'Max Verstappen': 'VER',
  'Sergio Pérez': 'PER',
  'Sergio Perez': 'PER',  // Some entries without accent
  'Lewis Hamilton': 'HAM',
  'George Russell': 'RUS',
  'Charles Leclerc': 'LEC',
  'Carlos Sainz': 'SAI',
  ...
  'Alexander Albon': 'ALB',
  'Nico Hülkenberg': 'HUL',
  'Nico Hulkenberg': 'HUL',  // Some entries without umlaut
  'Kevin Magnussen': 'MAG',
  'Zhou Guanyu': 'ZHO',
  'Valtteri Bottas': 'BOT',
  'Mick Schumacher': 'MSC',
...
async function parseF1Data2022() {
  console.log('\nParsing F1-Data-2022.csv...');

  const csvPath = path.join(process.cwd(), 'F1-Data-2022.csv');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isReadingResults && currentRace) {
      ...
      const driverName = parts[2 + offset];  // Already in English for 2022
      const teamName = parts[3 + offset];
      ...
      const driverCode = driverCodeMap[driverName] || '';
      const team = teamNameMap[teamName] || teamName;
...
function generateCSVFiles(races: RaceData[]) {
  ...
  const racesFile = path.join(process.cwd(), 'F1-2022-races.csv');
  ...
  const sessionsFile = path.join(process.cwd(), 'F1-2022-sessions.csv');
  ...
  const resultsFile = path.join(process.cwd(), 'F1-2022-results.csv');
  fs.writeFileSync(resultsFile, resultsCSV.join('\n'), 'utf-8');
  console.log(`  ✅ Created ${resultsFile} (${totalResults} results)`);

  console.log(`\n✅ Successfully generated 2022 CSV files`);
  console.log(`\nFiles created:`);
  console.log(`  - F1-2022-races.csv (${races.length} races)`);
  console.log(`  - F1-2022-sessions.csv (no sessions)`);
  console.log(`  - F1-2022-results.csv (${totalResults} results)`);
  console.log(`\nNext step: Run 'pnpm tsx server/scripts/parseYearlyCSV.ts 2022' to import into f1_data.json`);
}
